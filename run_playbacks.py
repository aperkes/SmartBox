...
def define_cages(corner_array):
    global default_window_dict, window_dict, camera_dict
...
def monitor_cameras(caps,visual = False):
    global n_cameras, n_cages, window_dict, cage_indices, monitor_duration, h, w
    counts = 100
    success = False
...
def select_song():
    song_log = './2019_log.csv'
    song_directory = './Songs/'
    song_df = pd.read_csv(song_log)
    next_row = song_df[song_df['Played'] == 0].iloc[0]
